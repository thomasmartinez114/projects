		...
		jQuery(".ult-new-ib").each(function(index, element) {
			...
			if(w_width>=768) {
				...
				if(banner_min_height != '') {
					...
					jQuery(this).css('min-height', banner_min_height);
				...
				jQuery(this).find(".ult-new-ib-img").css('max-width', 'initial');
			}
...
jQuery(document).ready(function(){
	function resize_uvc_map()
	{
		jQuery('.ultimate-map-wrapper').each(function(i,wrapelement){
			...
			jQuery('#'+map).css({'width':w, 'margin-left' : calculate_left });
		});
	}
	resize_uvc_map();
	jQuery(window).load(function(){
	...
	jQuery('.ui-tabs, .ui-accordion').bind('tabsactivate, accordionactivate', function(event, ui) {
