...
if(!class_exists('AIO_ultimate_exp_section')) 
{
...

	{
		...
		function ultimate_exp_scripts(){
			...
			wp_register_script("jquery_ultimate_expsection",plugins_url("../assets/min-js/expandable_section.min.js",__FILE__),array('jquery'),ULTIMATE_VERSION);
			wp_enqueue_script("jquery_ui",plugins_url("../assets/js/jquery-ui.js",__FILE__),array('jquery'),ULTIMATE_VERSION);
...
if($icon_type == 'custom'){
				...
				$newimg=wp_get_attachment_image_src( $new_icon_img, 'large');
				$newsrc=$newimg[0];
				$src1=$img[0];
				...
				if(!empty($img[0])){
					...
					$iconoutput .= "\n\t".'<img class="img-icon ult_exp_img '.$img_ext.'" alt="'.$alt.'" src="'.$img[0].'" />';	
					$iconoutput .= "\n".'</span>';
				}
				if(!empty($img[0])){
